...
import { fetchOrdDeProd, fetchReporteOrdDeProd } from '../../services/reporteService';
...
const ReporteOrdenDeProd = () => {
    ...
    const handleSearch = async () => {
        ...
        withSwalLoading('Cargando informaci\u00F3n')
        try {
            const data = await fetchOrdDeProd(form.empresa, form.sede, form.servicio, form.fecha);
            if (data === 0) {
                Swal.close();
                setLista([]);
    ...
    const handlePrint = async () => {
        withSwalLoading('Cargando informaci\u00F3n')
        try {
            const impData = await fetchReporteOrdDeProd(user, form.empresa, form.sede, form.servicio, form.fecha, "OrdProd")
            const printWindow = window.open('', '_blank');
            await openPdf('/Reportes/GetPdfOrdDeProd1', impData, printWindow);
            Swal.close();
        } catch (e) {
    ...
    const rendertabla = () => (
        <div className="tabla-container">
            ...
            >
                <Column field="Medicamento" header="Medicamento" style={{ width: '650px' }} />
                <Column field="Cantidad" header="# de dosis" style={{ width: '150px' }} />
                <Column field="Frecuencia" header="Frecuencia" style={{ width: '150px' }} />
                <Column field="Dosis" header="Dosis Unitaria" style={{ width: '180px' }} />
                <Column field="NomPaciente" header="Paciente" style={{ width: '350px' }} />
                <Column field="Habitacion" header="Habitaci&oacute;n" style={{ width: '150px' }} />
            </DataTable>
        </div>
    );
        ...
        <div>
            <div className="content" >
                <div className="btn-group flex-wrap">
                    ...
                    <Button label="Exportar" icon="pi pi-file-export" className="btn-lg btn-success" disabled={lista.length === 0} onClick={handlePrint} />
                </div>
